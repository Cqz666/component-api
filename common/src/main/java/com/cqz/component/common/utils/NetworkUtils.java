...
public class NetworkUtils {
  public static void main(String[] args) {
    System.out.println(getHostname2());
  }
...
//      s.connect(InetAddress.getByName("8.8.8.8"), 10002);
      ...
      throw new RuntimeException("Unable to find server port", e);
    }
    // fallback
    try {
      ...
      for (NetworkInterface ni : activeNetworkIFs) {
        ...
        if (addresses.size() > 0) {
          ...
          try {
            ...
            throw new RuntimeException("Unable to fetch raw IP address for: " + address);
          }
        }
      }
