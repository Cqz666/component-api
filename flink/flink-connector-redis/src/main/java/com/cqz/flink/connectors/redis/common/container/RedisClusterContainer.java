...
public class RedisClusterContainer implements RedisCommandsContainer, Closeable {
    ...
    @Override
    public void rpush(final String listName, final String value, final Integer ttl) {
        try {
            jedisCluster.rpush(listName, value);
            if (ttl != null) {
                jedisCluster.expire(listName, ttl);
            }
        } catch (Exception e) {
            if (LOG.isErrorEnabled()) {
    ...
    @Override
    public void lpush(String listName, String value, final Integer ttl) {
        try {
