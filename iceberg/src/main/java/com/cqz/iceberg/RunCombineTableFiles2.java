...
public class RunCombineTableFiles2 {
...
//                StreamExecutionEnvironment.getExecutionEnvironment();
        ...
        icebergMap.put("warehouse", "hdfs://qzcs240:9000/tmp/chenqizhu/warehouse/hadoop");

        CatalogLoader hadoop_catalog=CatalogLoader.hadoop("hadoop_catalog", new Configuration(), icebergMap);
        Catalog catalog=hadoop_catalog.loadCatalog();

        TableIdentifier tableIdentifier=TableIdentifier.of(Namespace.of("default"), "gamebox_event_iceberg");
...
//        deleteOldSnapshot(table);
    ...
    private static void combineFiles(StreamExecutionEnvironment env, Table table) {
        Actions.forTable(env,table)
                .rewriteDataFiles()
                .maxParallelism(1)
                .filter(Expressions.equal("datekey","20220425"))
